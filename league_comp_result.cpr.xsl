  ...
  -->
	...
	<xsl:template name="FsTask_list">
		<table class="fs_res">
          ...
          <th class="fs_res">
            <xsl:if test="/Fs/FsCompetition/@ftv_factor > 0">
              FTV Validity
            </xsl:if>
            <xsl:if test="/Fs/FsCompetition/@ftv_factor &lt;= 0">
              Day Quality
		...
		<tr class="fs_res_res_row" onmouseover="this.className = 'hover'" onmouseout="this.className='fs_res_res_row'" >
            ...
            <td class="fs_res" style="text-align: right">
              <xsl:choose>
                ...
                <xsl:otherwise>
                  <xsl:value-of select="@counting_points"/>/<del>
                    <xsl:value-of select="@points"/>
                  </del>
                </xsl:otherwise>
              </xsl:choose>
						...
						<h2>
 						...
 						<xsl:choose>
							<xsl:when test="$top_x_tasks='all'">
								<h3>
                  Total results <xsl:if test="/Fs/FsCompetition/@ftv_factor > 0">(FTV: <xsl:value-of select="100 * /Fs/FsCompetition/@ftv_factor"/>%)</xsl:if>
                </h3>
							</xsl:when>
							<xsl:otherwise>
								<h3>
						...
						<xsl:if test="string-length($filter_info) > 0">
							<p>
								<b>
									Results includes only those pilots where <xsl:value-of select="$filter_info"/>
								</b>
							</p>
						</xsl:if>
