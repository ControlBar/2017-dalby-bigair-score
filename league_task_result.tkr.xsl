...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  -->
  <xsl:param name="league_round_name"></xsl:param>
  ...
  <xsl:variable name="final_glide_decelerator" select="$task/FsScoreFormula/@final_glide_decelerator"/>
  ...
  -->
	...
	<xsl:template name="result_heading_row">
        ...
        <xsl:choose>
          <xsl:when test="$final_glide_decelerator = 'aatb'">
            <th class="fs_res">
              Alt.<br/>@ES
            </th>
            <th class="fs_res">
          ...
          <xsl:when test="$final_glide_decelerator = 'cess'">
            <th class="fs_res">
              Alt.<br/>@ES
            </th>
          </xsl:when>
  ...
  -->
	<xsl:template name="result_row">
        ...
        <xsl:choose>
          <xsl:when test="$final_glide_decelerator = 'aatb'">
            <td class="fs_res" align="right">
              <xsl:if test="$task_pilot_result/@altitude_at_ess != 0">
                <xsl:value-of select="$task_pilot_result/@altitude_at_ess"/>
              </xsl:if>
            </td>
            <td class="fs_res" align="right">
              <xsl:if test="$task_pilot_result/@altitude_bonus_seconds != 0">
                <xsl:value-of select="$task_pilot_result/@altitude_bonus_time"/>
              </xsl:if>
            </td>
            <td class="fs_res" align="right">
              <xsl:if test="$task_pilot_result/@scored_ss_time != '00:00:00'">
                <xsl:value-of select="$task_pilot_result/@scored_ss_time"/>
              </xsl:if>
            </td>
          </xsl:when>
          <xsl:when test="$final_glide_decelerator = 'cess'">
      ...
      <xsl:if test="$task_state = 'STOPPED' and $bonus_gr > 0">
        <td class="fs_res" align="right">
          <xsl:value-of select="format-number($task_pilot_result/@real_distance, concat('#0', $decimal_separator, '00'))"/>
        </td>
        <td class="fs_res" align="right">
          <xsl:choose>
            <xsl:when test="$task_pilot_result/@landed_before_stop = 'True'">
              0
            </xsl:when>
            <xsl:otherwise>
              <xsl:if test="$task_pilot_result/@last_altitude_above_goal > 0">+</xsl:if>
        ...
        <div class="fs_res">
				...
				<p style="font-size:xx-small">
					<xsl:value-of select="/Fs/FsCompetition[1]/@from"/> to <xsl:value-of select="/Fs/FsCompetition[1]/@to"/>
						...
						<h2>
              <xsl:value-of select="$league_round_name"/>
            </h2>
					<h3>
